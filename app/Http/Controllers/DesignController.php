...
class DesignController extends Controller
{
        ...
        if($request->ajax())
        {
            ...
            else
            {
                if(($request->start) && ($request->end)){
                    $product = DB::table('products')->where('status', 1)->whereBetween('selling_price', [$request->start, $request->end])->orderBy('id', 'DESC')->limit(6)->get(); 
                    // dd($request->start);
                }
                else{
    ...
    public function loadSubCategoryProduct(Request $request, $sub)
    {
        if($request->ajax())
        {
            if($request->id > 0)
            {
                $products = DB::table('products')->where('sub_category', $sub)->where('id', '<', $request->id)->where('status', 1)->orderBy('id', 'DESC')->limit(6)->get(); 
            }
            else
            {
                $products = DB::table('products')->where('sub_category', $sub)->where('status', 1)->orderBy('id', 'DESC')->limit(6)->get();
            }
            $output = '';
            $last_id = '';
            if(!$products->isEmpty())
